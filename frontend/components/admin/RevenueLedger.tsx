          ...
          <tbody className="divide-y divide-slate-200 dark:divide-slate-800/60 text-slate-800 dark:text-slate-200">
            {ledgerData.map((row) => (
              <tr key={row.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors">
                ...
                <td className="p-4 font-bold text-slate-900 dark:text-white">${row.grossRentalUSDC.toFixed(2)}</td>
                <td className="p-4 text-amber-700 dark:text-amber-400 font-extrabold">+${row.treasuryFee2USDC.toFixed(2)}</td>
                <td className="p-4 text-emerald-700 dark:text-emerald-400 font-bold">${row.netPayoutUSDC.toFixed(2)}</td>
