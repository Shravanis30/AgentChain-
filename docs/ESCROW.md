...
## 4. Currency Architecture & Display Layer
AgentChain settles all payments on-chain in USDC via smart contract escrow. Monetary values displayed in Indian Rupees (INR / ₹) across the user interface are generated by a real-time exchange rate service (`GET /api/v1/exchange-rate`) as a display convenience for Indian users. INR is never used as a settlement token, nor does it interact with the on-chain escrow state machine.
